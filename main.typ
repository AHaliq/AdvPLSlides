...
#show: metropolis-theme.with(
  ...
  config-info(
    ...
    subtitle: [HoTT from scratch @daniel],
    author: [Abdul Haliq],
    date: datetime.today(),
    institution: text(
            size: 1.2em,
...
- *Strong Types* with features such as generics allows more expressive guarantees i.e. $"Vec"(X)$, $"Vec"("Int")$ vs $"Vec"$ #pause
...
- *Homotopy Type Theory* and univalence asserts that when types are equivalent we automatically have a map for terms from one type to the other. Thus given a proof / term in one type, for free we get the proof in the other; proof transfer #pause
...
- To motivate proof transfer, consider the following example
...
#figure(
  lefttable(
    ...
    pause,
    [Computation / $beta$],
    $iota_Gamma comp iota_Gamma^(-1) = id$,
    pause,
    [Uniqueness / $eta$],
    $iota_Gamma^(-1) comp iota_Gamma = id$,
  ),
)
  ...
  grid(
    ...
    figure(
      proof-tree(
        rule(
          name: [$Sigma$-$beta_2$],
          $Gamma hy upright(bold("snd"))(upright(bold("pair"))(a,b)) = b : B[id. a]$,
          $Gamma hy a : A$,
          $Gamma hy b : B[id. a]$,
        ),
      ),
...
```coq
Definition snd {A : Type} {B : A -> Type} (p : Σ A B) : B (fst p) :=
  match p with
  | pair _ b => b
  end.
```
#meanwhile
#align(center)[#text(size:0.75em)[_based on code from @poly _]]
...
- Our theory with equality reflection can be modelled with sets
- $[[Gamma hy A Ty]]$ is the set modelling the type $A$ in context $Gamma$ #pause
$
  [[Gamma hy a : A]] &= [[a]] in [[Gamma hy A Ty]] \
  [[Gamma hy Eq(A,a,b) Ty]] &= { star | [[Gamma hy a : A]] and [[Gamma hy b : A]] and [[a]] ~_(Gamma,A) [[b]] } \
  [[a]] ~_(Gamma,A) [[b]] &= exists! C in [[Gamma hy A Ty]]_(slash ~). [[a]] in C and [[b]] in C
$
#pause
- thus denotationally the isomorphism also holds
$
  Tm(Gamma, Eq(A,a,b)) iso {star | a = b} iso [[Gamma hy Eq(A,a,b) Ty]]
$
- we can see that $[[p : Eq(A,a,b)]] and [[q : Eq(A,a,b)]] => [[p]] = [[q]] = star$ #pause
- when any two proof of equalities are equal, we call the equality proof irrelevant
- this is also called the property of *Uniqueness of Identity Proofs (UIP)*
...
#pause
- *Normalization*: with UIP our types aren't injective. Injective types are almost always the consequence of a proof of normalization which are used in decidable typechecking. #pause; (we won't explore why here, but lets take it for granted for now)
#pause
#align(center)[_We want a theory with decidable type checking to implement!_]
...
$
  {c in Tm(Gamma. Unit, C) | c[id. tt]} iso {star}
$
#pause
$
  rec(tt, c) = c: C[id. tt]
...
#figure(
  ...
  -> & B #h(0.25em) b \
    $,
    $
      &trans p#h(0.25em) q = subst\ #pause
    &#h(1em) lambda x. Id(A,a,x) \ #pause
    &#h(1em) q \ #pause
    &#h(1em) p
    $,
  ),
...
#figure(
  ...
  -> & B #h(0.25em) b \
    $,
    $
      &cong f#h(0.25em) p = subst \ #pause
    &#h(1em) lambda x. Id(B,f#h(0.25em) a, f#h(0.25em) x) \ #pause
    &#h(1em) p \ #pause
...
$
  jrule => uniq
$
#align(center)[_let $[z] = Sigma(z : A, Id(A,x,z))$; points identified with $z$_]
#figure(
  grid(
    columns: 2,
    align: (left + top, left + top),
...
$
  "Funext" = (A: UU) -> (B: A -> UU) -> (f, g: (a: A) -> B#h(0.25em) a) \ -> (
    (a : A) -> Id(B#h(0.25em) a, f#h(0.25em) a, g #h(0.25em) a)
  ) -> Id((a: A) -> B#h(0.25em) a, f, g)
$ #pause
...
#figure(
  diagram(
    ...
    $
      Tm(Gamma, UU)
      edge("r", El, "->", bend: #20deg)
      edge("r", code, "<-", bend: #{-20deg}) &
      Ty(Gamma)
    $,
  ),
)
...
$
  {f in Tm(Gamma. Id(U, A, B), C) | u} iso {star}
$
- we want $f$ to work as a "bridge" that brings proofs in $A$ to $B$, and vice versa
#figure(
  diagram(
    cell-size: 10mm,
    $
      A
  #edge("rr", $p$, "->", shift: 0pt, label-anchor: "south", label-sep: 0em, bend: 30deg)
  #edge("rr", $f$, "<-->", shift: 0pt, label-anchor: "north", label-sep: 0em, bend: -30deg)
...
#pause
- the universe is univalent when $f : C$ induces $u : C$; the elim and intro rules
$
  u : C(p : Id(U,A,B),f : C(A,B))
$
#pause
...
$
  { f : Tm(Gamma. Id(HProp, A, B), iso ) | "propUnivalence" } iso {star} \ #pause
$
$
...
$
  { f : Tm(Gamma. Id(UU, A, B), equiv) | ua } iso {star} \ #pause
...
- Revisiting our motivating example of $bb(N)$ and $N$
- Given $"isEven"_bb(N) : bb(N) -> Bool$ and $(f,g, \_ ) : N iso bb(N)$ #pause
$
  "isEven"_N &: N -> Bool &&= "isEven"_bb(N) comp (
    upright(bold("fst")) comp ua comp ua^(-1) #h(0.25em) (f, lambda b. ((..., ...), lambda y. ... ))
  )
$
- Without "using" the structure of $N$ we get $"isEven"_N$ for free! This is called proof transfer
#pause
- $ua$ is an axiom, thus when encountered in a prover, it is stuck and irreducible
...
#pause
$
  {f in Tm(Gamma. Id(UU, A, B), tilde.eq) | ua} iso {star} \
  ua : Id(UU, A, B) tilde.eq (A tilde.eq B)
$
- *HoTT*: univalence theoretically allows us to do proof transfer
